...
(function (e) {
  ...
  const m = () => "hidden" !== document.visibilityState || document.prerendering ? 1 / 0 : 0,
    ...
    y = () => {
      if (l < 0) {
        const e = document.prerendering ? void 0 : globalThis.performance.getEntriesByType("visibility-state").filter(e => "hidden" === e.name)[0]?.startTime;
        l = e ?? m(), v(), n(() => {
          setTimeout(() => {
            l = m(), v();
          });
  ...
  const B = e => {
      ...
      "hidden" === document.visibilityState ? e() : (t(e = u(e)), document.addEventListener("visibilitychange", e, {
        once: true
      }));
    },
    N = [200, 500];
