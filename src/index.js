...
(function () {
  ...
  function getSourceContext(el) {
    ...
    return block && `.${block.getAttribute('data-block-name')}` || walk(el, isDialog) && 'dialog' || walk(el, e => e.tagName && e.tagName.includes('-') && e.tagName.toLowerCase()) || ['nav', 'header', 'footer', 'aside'].find(t => el.closest(t)) || walk(el, e => e.id && `#${e.id}`);
  }
  ...
  const PLUGINS = {
    ...
    onetrust: {
      ...
      when: () => document.querySelector('#onetrust-consent-sdk') || hasCookieKey('OptanonAlertBoxClosed'),
      isBlockDependent: true
    }
  };
  ...
  function loadPlugin(key, params) {
    ...
    return pluginLoadPromise.then(p => p.default && p.default(params) || typeof p === 'function' && p(params)).catch(() => {});
  }
  ...
  function addLoadResourceTracking() {
    const observer = new PerformanceObserver(list => {
      try {
        list.getEntries().filter(e => !e.responseStatus || e.responseStatus < 400).filter(e => window.location.hostname === new URL(e.name).hostname || fflags.has('allresources')).filter(e => new URL(e.name).pathname.match('.*(\\.plain\\.html$|\\.json|graphql|api)')).forEach(e => {
          sampleRUM('loadresource', {
            source: e.name,
            target: Math.round(e.duration)
          });
        });
        list.getEntries().filter(e => e.responseStatus >= 400).filter(e => !new URL(e.name).pathname.match('.*(/\\.rum/1[0-9]{0,3})')).forEach(e => {
